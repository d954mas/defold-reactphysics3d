...
return function()
	describe("AABB", function()
		...
		test("testCollision()", function()
			...
			local quat = vmath.quat()
			local aabb = box:computeAABB(pos, quat)
			local aabb_collide = box:computeAABB(vmath.vector3(0.5, 0.5, 0.5), quat)
			local aabb_notcollide = box:computeAABB(vmath.vector3(2.01, 0, 0), quat)
		...
		test("mergeWithAABB()", function()
			...
			local quat = vmath.quat()
			local aabb = box:computeAABB(pos, quat)
			local aabb2 = box:computeAABB(vmath.vector3(10, 0, 5), quat)
		...
		test("mergeTwoAABBs()", function()
			...
			local quat = vmath.quat()
			local aabb = box:computeAABB(pos, quat)
			local aabb_1 = box:computeAABB(vmath.vector3(10, 0, 5), quat)
			local aabb_2 = box:computeAABB(vmath.vector3(-10, 0, 10), quat)
		...
		test("contains()", function()
			...
			local quat = vmath.quat()
			local aabb = box:computeAABB(pos, quat)
			local aabb_small = box:computeAABB(pos, quat)
			aabb_small:inflate(-0.2, -0.2, -0.2)
			local aabb_big = box:computeAABB(pos, quat)
