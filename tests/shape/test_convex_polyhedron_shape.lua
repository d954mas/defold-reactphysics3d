...
return function()
	describe("ConvexPolyhedronShape", function()
		...
		test("getCentroid()", function()
			assert_equal_v3(shapes.box:getCentroid(), vmath.vector3(0))
		end)

		test("findMostAntiParallelFace()", function()
			assert_equal(shapes.box:findMostAntiParallelFace(vmath.vector3(1, 0, 0)), 3)
		end)
	end)
end
