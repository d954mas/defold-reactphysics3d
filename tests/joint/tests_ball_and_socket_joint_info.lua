...
return function()
	describe("BallAndSocketJointInfo", function()
		...
		test("create world position", function()
			...
			assert_not_nil(info)
			pprint(info)
			TEST_JOINT_INFO.test_joint_info(info, { body1 = body1, body2 = body2, type = rp3d.JointType.BALLSOCKETJOINT })
		...
		test("create local position", function()
			local info = rp3d.createBallAndSocketJointInfoLocalSpace(body1, body2, vmath.vector3(0, -1, 0), vmath.vector3(0, 1, 0))
			assert_not_nil(info)
			TEST_JOINT_INFO.test_joint_info(info, { body1 = body1, body2 = body2, type = rp3d.JointType.BALLSOCKETJOINT })
			assert_true(info.isUsingLocalSpaceAnchors)
			assert_equal_v3(info.anchorPointBody1LocalSpace, vmath.vector3(0, -1, 0))
			assert_equal_v3(info.anchorPointBody2LocalSpace, vmath.vector3(0, 1, 0))
